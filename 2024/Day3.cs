...
namespace AdventOfCode
{
    public class Day3
    {
        private static readonly string enable = "do()";
        private static readonly string disable = "don't()";
        ...
        public static int Main3(string[] args)
        {
            ...
            while (index < s.Length)
            {
                if (!enabled)
                {
                    if (s.Length >= index + Day3.enable.Length && s.Substring(index, Day3.enable.Length) == Day3.enable)
                    {
                        index += Day3.enable.Length;
                        enabled = true;
                    }
                    else
                    {
                ...
                if (enabled && s.Length >= index + Day3.disable.Length && s.Substring(index, Day3.disable.Length) == Day3.disable)
                {
                    index += Day3.disable.Length;
                    enabled = false;
                    continue;
                }

                if (Multiplication.TryParse(s, index, out multiplication, out index))
                {
        ...
        private class Multiplication
        {
            ...
            private static readonly string preamble = "mul(";
            private static readonly int operandMaxLength = 3;
            private static readonly char terminatorX = ',';
            private static readonly char terminatorY = ')';
            ...
            {
                return Multiplication.preamble + x.ToString() + Multiplication.terminatorX + y.ToString() + Multiplication.terminatorY;
            }

            public static bool TryParse(string s, int index, out Multiplication multiplication, out int nextIndex)
            {
                multiplication = null;
                nextIndex = index + 1;

                if (s.Length < index + Multiplication.preamble.Length || s.Substring(index, Multiplication.preamble.Length) != Multiplication.preamble)
                {
                    return false;
                }

                int x, y;
                if (!Multiplication.TryParse(s, index + Multiplication.preamble.Length, Multiplication.terminatorX, Multiplication.operandMaxLength,
                                             out x, out index)
                 || !Multiplication.TryParse(s, index, Multiplication.terminatorY, Multiplication.operandMaxLength,
                                             out y, out index))
                {
                    return false;
                }
                ...
                nextIndex = index;
                multiplication = new Multiplication(x, y);
                return true;
            }
            
            private static bool TryParse(string s, int index, char terminator, int maxDigits, out int operand, out int nextIndex)
            {
                operand = 0;
                nextIndex = index+1;
                ...
                while (i < s.Length && i < index + maxDigits + 1)
                {
                    if (s[i] == terminator)
                    {
                        terminated = true;
